...
| 1.3 | Select testers | ⚠️ | **Internal track rejects Google Groups** (API returns 400: "Cannot set tester group on an internal track"). Browser only: Play Console → Internal testing → Testers → Create email list. **For automation: skip internal and use alpha — see `docs/play/AUTOMATION_ENVELOPE.md`.** |
...
| 3.4 | Select testers | ✅ | `make testers TRACK=alpha ARGS="--set-groups vagarylabs-tester@googlegroups.com"` — Google Groups attached via API. **This is the recommended tester-management pattern** (see `docs/play/AUTOMATION_ENVELOPE.md`). Individual email add/remove goes through groups.google.com OR Admin SDK if on Workspace. |
...
See `docs/play/AUTOMATION_ENVELOPE.md` for the full honest scope. Summary:

**Total: ~30-35 min browser time, once forever per app.**

1. Click **Create app** — 1 min
2. Fill declarations (transcribe from DECLARATIONS.md) — 25-30 min (Data safety is the long one)
3. Paste privacy policy URL — 30 sec
4. Pick category + tags — 1 min
5. **(Optional — skip if using alpha as the first test track)** Click Start rollout to Internal testing — 1 min
6. `make testers TRACK=alpha ARGS="--set-groups <your-group>@googlegroups.com"` — scripted, first time only

**After that**: every future release across every track is `make release-internal` / `make promote-*` / tag-push CI. Adding new testers is "add email to Google Group" (groups.google.com UI, 30 sec) — never Play Console again.

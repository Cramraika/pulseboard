...
11. **First "Start rollout to Internal testing" click** — Internal testing → Releases. API returns 400 "Only releases with status draft may be created on draft app" until this is done once per app. After that, fully automated forever. **Confirmed 2026-04-22**: filling all declarations is necessary but NOT sufficient — the click itself is Google's mandated human-attestation step. No API workaround. **Confirmed 2026-04-24**: this gate blocks ALL `edits.commit` calls, not just release rollouts. Listing changes, contact-details changes, image uploads all fail at commit once declarations are filled until the click happens. So first-time setup order matters: push everything (listings, details, images) BEFORE filling all declarations, not after.
